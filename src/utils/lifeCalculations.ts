...
export interface LifeData {
  ...
  breaths: number;
  sleepHours: number;
  worldPopAtBirth: number;
  currentWorldPop: number;
  peopleMet: number;
  totalBirths: number;
  totalDeaths: number;
  distanceTraveledAroundSun: number;
  distanceThroughGalaxy: number;
  lunarCycles: number;
  tripsAroundSun: number;
  sequoiaPercent: string;
  lifespanPercent: string;
}

export const calculateLifeData = (birth: string | Date, expectancy: number): LifeData => {
  let birthDateTime: Date;
  if (typeof birth === 'string' && birth.includes('-')) {
    const [y, m, d] = birth.split('-').map(Number);
    const dateUTC = new Date(Date.UTC(y, m-1, d, 0, 0, 0));
    birthDateTime = new Date(dateUTC.getTime() + (5.5 * 3600000));
  } else {
    birthDateTime = new Date(birth);
  }
  ...
  const timeDiff = now.getTime() - birthDateTime.getTime();
...
export const getWorldPopulation = (year: number) => {
  const popData: { [key: number]: number } = {
...
export const getWeekDate = (weekNum: number, birthDate: Date) => {
